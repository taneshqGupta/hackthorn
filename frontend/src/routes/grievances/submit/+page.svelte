<script lang="ts">
	...
	async function handleSubmit() {
		...
		if (title.length < 10) {
			console.log(
				"[SUBMIT] Validation failed: Title too short:",
				title.length,
			);
		...
		if (description.length < 20) {
			console.log(
				"[SUBMIT] Validation failed: Description too short:",
				description.length,
			);
		...
		try {
			...
			console.log(
				"[SUBMIT] Grievance created successfully, ID:",
				grievanceId,
			);
			...
			if (selectedFiles.length > 0 && grievanceId) {
				...
				console.log(
					"[SUBMIT] Posting photos to /api/grievances/" +
						grievanceId +
						"/photos",
				);
				await api.post(
					`/api/grievances/${grievanceId}/photos`,
					formData,
				);
			...
			console.log(
				"[SUBMIT] Submission complete, redirecting to grievances list",
			);
			success = true;
			setTimeout(() => {
				goto("/grievances");
	...
	function nextPage() {
		...
		const pageIndex = ["details", "category", "upload", "review"].indexOf(
			currentPage,
		);
		console.log("[SUBMIT] Current page index:", pageIndex);
		if (pageIndex < 3) {
			const newPage = ["details", "category", "upload", "review"][
				pageIndex + 1
			];
	...
	function prevPage() {
		...
		if (pageIndex > 0) {
			currentPage = ["details", "category", "upload", "review"][
				pageIndex - 1
			];
		}
	}
	...
	<div class="pda-content">
		...
		<div class="pda-grid">
			{#each categories as cat}
				<button
					class="pda-card {category === cat.value
						? 'pda-card--active'
						: ''}"
		...
		<div class="pda-grid">
			{#each priorities as pri}
				<button
					class="pda-card pda-card--small {priority === pri.value
						? 'pda-card--active'
						: ''}"
				...
				{#each filePreviews as preview, i}
					<div class="pda-preview">
						...
						<button
							...
							class="pda-preview-remove">×</button
						>
					</div>
				{/each}
			</div>
	...
	<div class="pda-content">
		...
		<div class="pda-review">
			...
			<div class="pda-box" style="margin-top: 12px;">
				<div class="pda-label">TITLE</div>
				<div
					style="font-weight: 900; font-size: 10px; margin-top: 4px;"
				>
					{title || "[No title]"}
				</div>
			</div>
			...
			<div class="pda-box" style="margin-top: 8px;">
				<div class="pda-label">DESCRIPTION</div>
				<div
					style="font-weight: 900; font-size: 10px; margin-top: 4px; line-height: 1.4;"
				>
		...
		<div class="pda-nav">
			<button onclick={prevPage} class="pda-btn" disabled={loading}
				>← BACK</button
			>
			<button
				onclick={handleSubmit}
				class="pda-btn pda-btn--primary"
