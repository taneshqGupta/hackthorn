<script lang="ts">
    ...
    async function loadAttendance() {
        try {
            ...
            const res = await api.get<ApiResponse<AttendanceSummary[]>>('/api/attendance/all');
...
<style>
    ...
    .title { font-size: 3rem; font-weight: 950; text-transform: uppercase; margin: 0; }
    .subtitle { font-weight: 700; text-transform: uppercase; opacity: 0.7; margin-bottom: 2rem; }
    ...
    .course-header h3 { font-weight: 900; margin-bottom: 1.5rem; text-decoration: underline; }
    ...
    .table-header { font-weight: 900; padding: 0.5rem 0; border-bottom: 2px solid #000; }
    .table-row {
        display: grid;
    ...
    .status.present { color: #059669; font-weight: 900; }
