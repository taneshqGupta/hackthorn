<script lang="ts">
	...
	function handleUpvote(e: MouseEvent | KeyboardEvent) {
		console.log('[PostCard] upvote clicked for', id);
		// stopPropagation already handled by modifier, keep defensively
		e.stopPropagation?.();
...
<div class="card" onclick={() => onclick?.()} role="button" tabindex="0" onkeydown={(e) => (e as KeyboardEvent).key === 'Enter' && onclick?.()}>
	...
	<div class="footer">
<div class="footer-item upvote-btn" onclick={(e) => { (e as MouseEvent).stopPropagation(); handleUpvote(e as MouseEvent); }} role="button" tabindex="0" onkeydown={(e) => { (e as KeyboardEvent).stopPropagation(); if ((e as KeyboardEvent).key === 'Enter') handleUpvote(e as KeyboardEvent); }}>
