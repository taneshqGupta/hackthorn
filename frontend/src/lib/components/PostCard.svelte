<script lang="ts">
    ...
    // 1. We only track if the user has *changed* their mind locally
    // undefined = user hasn't touched it, use server data
    let userHasLiked = $state<boolean | undefined>(undefined);

    // 2. Compute the final "Liked" status
    // If user clicked (userHasLiked is set), use that. Otherwise use prop (isLiked).
    let currentLiked = $derived(userHasLiked ?? isLiked);

    // 3. Compute the final "Count"
    // We take the server count and adjust it only if our local state differs from the server
    let currentUpvotes = $derived.by(() => {
        let count = upvotes;
        if (currentLiked && !isLiked) count += 1; // User liked, server doesn't know yet
        if (!currentLiked && isLiked) count -= 1; // User unliked, server thinks liked
        return count;
    });
    ...
    function handleUpvote(e: MouseEvent | KeyboardEvent) {
        ...
        // Toggle the local override
        userHasLiked = !currentLiked;
        
        console.log(`[PostCard] Toggled like for ${id}. New state: ${userHasLiked}`);
        if (onupvote) onupvote(userHasLiked);
    }

    // ... (rest of your helper functions like formatDate) ...
    ...
    <div class="body">
        ...
        {#if displayImages.length > 0}
            <div class="images" class:single={displayImages.length === 1} class:grid={displayImages.length > 1}>
                {#each displayImages as image} <img src={image} alt="Post attachment" /> {/each}
            </div>
        {/if}
    </div>
...
<style>
    /* ... (All your styles remain exactly the same as before) ... */
    .card {
        position: relative;
    ...
    /* Style for the active like state */
    .upvote-btn.liked {
        color: #e11d48;
    }
